...
function Runtime(props) {
  ...
  const [locatedElements, setLocatedElements] = createSignal([]);
  ...
  function keyDownListener(e) {
    if (props.usageQueryString) {
      setLocatedElements(Array.from(document.querySelectorAll(props.usageQueryString)));
    }
  ...
  return [_$memo((() => {
    ...
    const _c$2 = _$memo(() => !!(!props.usageQueryString && holdingModKey() && currentElement()), true);
    ...
    const _c$3 = _$memo(() => !!(props.usageQueryString && holdingModKey() && locatedElements().length), true);

    return () => _c$3() && locatedElements().map(el => _$createComponent(UsageOutline, {
      currentElement: el,
      showTreeFromElement: showTreeFromElement,
...
export function initRender(solidLayer, adapter, targets, usageQueryString) {
